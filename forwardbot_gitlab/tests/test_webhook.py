...
class TestWebhook(HttpCase, ForwardBotCase):
    ...
    def send_payload(self, payload):
        return self.url_open(
            "/fwbot", data=json.dumps(payload), headers=self.gitlab_headers
        )
    ...
    def test_create_mr_invalid_branch(self):
        branch = "16.0-random-fix"
        payload = self._gen_payload(99, target_branch=branch)
        with self.assertLogs() as logs:
            response = self.send_payload(payload)
            self.assertTrue(f"{branch} not eligible" in logs[1][0])
            self.assertEqual(200, response.status_code)
    ...
    def test_update_mr(self):
        ...
        updated_payload = self._gen_payload(
            55, action="update", title="Just updated this"
        )
        response = self.send_payload(updated_payload)
        self.assertEqual(200, response.status_code)
        self.assertMergeRequestEquals(
            updated_payload, self.model_mr.browse([merge_request.id])
        )
